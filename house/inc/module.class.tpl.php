...
</script><textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:{$rs[field_inputwidth]}px;height:{$rs[field_inputheight]}px;visibility:hidden;'>\$rsdb[{$rs[field_name]}]</textarea>";
	}
...
function show_tpl($rs,&$tplcode){
	...
	$rs[form_units] && $show .=" {\$module_DB[\$rsdb[mid]][field][$rs[field_name]][form_units]}";
