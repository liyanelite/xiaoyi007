...
if($action=='mod'){
	...
	if($moduleid==2){
		$SQL = "SELECT A.*,B.* FROM {$_pre}person A LEFT JOIN {$_pre}content_$moduleid B ON A.id=B.id $SQL ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}else{
		$SQL = "SELECT A.*,B.*,C.title AS companyname FROM {$_pre}content A LEFT JOIN {$_pre}content_$moduleid B ON A.id=B.id LEFT JOIN {$pre}hy_company C ON A.uid=C.uid $SQL ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}
