...
function Module_Field($path=''){
	...
	$this->style = $STYLE;
	$this->f_path = $path?$path:'../';
	$this->tpl_index_new = $this->f_path."template/{$this->style}/";
	$this->tpl_admin_new = $this->f_path."template/admin/";
	$this->tpl_member_new = $this->f_path."template/member/";
}
...
function delete_module($id){
	...
	@unlink($this->tpl_admin_new."list_$id.htm");
	@unlink($this->tpl_member_new."list_$id.htm");
}
...
function make_template($id){
	...
	$adminlist_tpl = $tplDB->read_tpl("list_{$_type}.htm",$tpldb['admin_list'],'admin');

	$memberlist_tpl = $tplDB->read_tpl("list_{$_type}.htm",$tpldb['member_list'],'member');
	...
	foreach($field_db AS $key=>$rs){
		...
		$rs[listfilter] && $tplDB->listfilter_tpl($rs,$field_db,$adminlist_tpl);
		$rs[listfilter] && $tplDB->listfilter_tpl($rs,$field_db,$memberlist_tpl);

		if($rs[listshow]){
			$tplDB->list_tpl($rs,$indexlist_tpl);	//前台列表页
			$tplDB->list_tpl($rs,$indexsearch_tpl);	//前台搜索页

			$tplDB->list_tpl($rs,$adminlist_tpl);
			$tplDB->list_tpl($rs,$memberlist_tpl);
		}
		if($rs[search]){
	...
	$tplDB->write_tpl("list_$id",$adminlist_tpl,'admin');
	$tplDB->write_tpl("list_$id",$memberlist_tpl,'member');
}
