...
while($rs = $db->fetch_array($query))
{
	...
	if($timestamp-$rs[posttime]<(3600*$webdb[UpdatePostTime])){
		...
		$rs[update]="<A HREF=\"$webdb[www_url]/$type/job.php?job=update&fid=$rs[fid]&id=$rs[id]\">刷新</A>";
	}
	if($rs['list']>$timestamp){
		$rs[dotop]='<A HREF="#" style="color:#ccc;" onclick="alert(\'已经置顶了\')">置顶</A>';
	}else{
		$rs[dotop]="<A HREF=\"$webdb[www_url]/$type/job.php?job=dotop&fid=$rs[fid]&id=$rs[id]\">置顶</A>";
	}
	$rs[pop1]="<A HREF=\"$webdb[www_url]/$type/job.php?job=popshow&type=1&fid=$rs[fid]&id=$rs[id]\">首页焦点</A>";	
	$rs[pop2]="<A HREF=\"$webdb[www_url]/$type/job.php?job=popshow&type=2&fid=$rs[fid]&id=$rs[id]\">栏目焦点</A>";
	$rs[pop3]="<A HREF=\"$webdb[www_url]/$type/job.php?job=popshow&type=3&fid=$rs[fid]&id=$rs[id]\">分类焦点</A>";
