...
if($job=="listsort")
{
	...
	if(is_file("../cache/table.txt")){
		$url=read_file("../cache/table.txt");
...
elseif($action=='table')
{
	...
	if($ckk){
		$page++;
		write_file("../cache/table.txt","$admin_path&action=$action&tableid=$tableid&tableid0=$tableid0&fid=$fid&page=$page");
