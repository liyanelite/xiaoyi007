...
if($action=="search")
{
	...
	if(!$mid||!is_table("{$_pre}content_$mid"))
	{
		...
		/*分页*/
		$showpage=getpage("{$_pre}content A","WHERE  $_SQL","?fid=$fid&keyword=$keyword&action=search&type=$type$search_url",$rows);

		$SQL="SELECT * FROM {$_pre}content A WHERE $_SQL ORDER BY A.posttime DESC LIMIT $min,$rows ";
	}
	else
	{
		if($keyword){
		...
		$SQL="SELECT A.*,B.* FROM {$_pre}content A LEFT JOIN {$_pre}content_$mid B ON A.id=B.id WHERE A.mid='$mid' AND $_SQL ORDER BY A.posttime DESC LIMIT $min,$rows ";
	}
